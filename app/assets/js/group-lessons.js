...
const renderCalendar = () => {
  ...
  for (let x = firstDayIndex; x > 0; x--) {
    days += `<div class="prev-date" data-year=${date.getFullYear()} data-month=${date.getMonth()} data-date=${prevLastDay - x + 1
      }>${prevLastDay - x + 1}</div>`;
  }
  ...
  for (let i = 1; i <= lastDate; i++) {
    if (i === new Date().getDate() && date.getMonth() === new Date().getMonth()) {
      days += `
      <div class="today flex-column border-1 border-primary-lightest" h-100" data-year=${date.getFullYear()} data-month=${date.getMonth() + 1
        } data-date=${i}>
      <p class="calendar-today rounded-3 bg-primary p-1">${i}</p>
      </div>
      `;
    } else {
      days += `<div class="flex-column border-1 border-primary-lightest" data-year=${date.getFullYear()} data-month=${date.getMonth() + 1
        } data-date=${i}>${i}</div>`;
    }
  }
  ...
  for (let j = 1; j <= nextDays; j++) {
    days += `<div class="next-date" data-year=${date.getFullYear()} data-month=${date.getMonth() + 2
      } data-date=${j}>${j}</div>`;
  }
...
function takeDataToReserveForm() {
  ...
  groupLessonReserveBtn.forEach((item) => {
    item.addEventListener('click', (e) => {
      ...
      const targetDate = `${date.getFullYear()} 年 ${e.target.parentNode.parentNode.dataset.month
